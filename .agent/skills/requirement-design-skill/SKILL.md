...
**记住用户的操作系统，后续所有涉及系统操作的地方（文件路径、终端命令、环境配置等）都必须给出对应 OS 的正确格式：**

| 场景       | macOS / Linux         | Windows            |
| ---------- | --------------------- | ------------------ |
| 路径分隔符 | `/`                   | `\`                |
| 用户目录   | `~/` 或 `/Users/xxx/` | `C:\Users\xxx\`    |
| 打开文件   | `open [文件]`         | `start [文件]`     |
| 创建目录   | `mkdir -p`            | `mkdir`            |
| 环境变量   | `export KEY=VALUE`    | `set KEY=VALUE`    |
| 包管理器   | `brew` / `apt`        | `winget` / `choco` |
...
6. **PDF 兼容性（必须遵守）**：生成的文档内容必须确保在转换为 PDF 时不会出现布局错乱。**严格禁止**以下内容：
   - **禁止使用 Unicode 制表符/箱线字符绘制图表**：如 `┌ ┐ └ ┘ │ ─ ├ ┤ ┬ ┴ ┼ ▼ ▲ ► ◄` 等字符。这些字符在 LaTeX/PDF 引擎中宽度不一致，会导致对齐完全错乱
   - **禁止使用 ASCII Art 图表**：任何依赖字符对齐的"文字画"架构图、流程图、拓扑图等都不允许使用，尤其是中文与 ASCII 混排时（CJK 字符在等宽字体中占双倍宽度，必然错位）
   - **禁止在代码块中放置非代码内容的对齐图形**：代码块（` ``` `）应仅用于展示真实代码或命令，不得用于承载需要视觉对齐的图表
   - **替代方案**：所有架构图、流程图、关系图等，必须使用以下 PDF 友好的方式表达：
     - ✅ **Markdown 表格**：用表格呈现分层架构、组件列表、模块对照等
     - ✅ **层级标题 + 列表**：用标题层级和缩进列表表达层次关系
     - ✅ **引用块（blockquote）**：用于标注层间通信协议、备注信息等
     - ✅ **加粗/斜体文字描述**：用结构化文字替代视觉图形
