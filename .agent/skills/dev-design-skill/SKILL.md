...
6. **前端通用技术栈确认**：
   ...
   在进入模块设计之前，向用户确认前端通用技术栈（仅首次需要确认，后续模块复用）：

   > **注意：如果用户选择了"仅后端设计"，跳过此步骤。**

   ```
   请确认前端通用技术栈配置：

   1. 🎨 CSS 方案？
      a. TailwindCSS（推荐，快速开发）
      b. CSS Modules
      c. Sass / Less
   ...
   2. 📦 UI 组件库？
      a. Ant Design（推荐，功能全面）
      b. Element Plus（Vue 生态）
      c. Arco Design
      d. Shadcn/ui
      e. 不使用组件库
      f. 其他：___
   ...
   6. 🔧 其他常用工具？以下是否需要？
      ...
      - lodash-es（工具函数）      ❓ 是否需要？
      - i18n（国际化）             ❓ 是否需要？
      - xlsx / file-saver（导出）  ❓ 是否需要？

   可以逐一回答，也可以说"全部默认"或"由你决定"。
   也可以说"不需要通用技术栈配置"，我会根据每个模块的需要自行选择。
   ```

   **如果用户选择"由你决定"或"全部默认"，使用以下默认配置：**

   | 配置项    | 默认值                                   |
   | --------- | ---------------------------------------- |
   | CSS 方案  | TailwindCSS                              |
   | UI 组件库 | Ant Design（React）/ Element Plus（Vue） |
   | HTTP 请求 | Axios                                    |
   | 表单方案  | 组件库自带表单                           |
   | 图表库    | 按需选择                                 |
   | 工具库    | dayjs                                    |

   **如果用户选择"不需要通用技术栈配置"，则在每个模块设计时根据具体需求自行推荐，不做全局约束。**

   确认后，将此配置应用于后续所有模块的前端设计中。
